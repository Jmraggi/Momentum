...
export function useWeightMutations(userId: string | undefined) {
  ...
  const create = useMutation({ mutationFn: async ({ metricId, input }: { metricId: string; input: WeightInput }) => { const source = await manualSource(); return requireData<WeightEntry>(await supabase.from('metric_entries').insert({ user_id: userId!, metric_id: metricId, data_source_id: source.id, numeric_value: input.value, recorded_at: input.recordedAt, metadata: metadata(input.note) }).select().single()) }, onSuccess: invalidate, retry: false })
  const update = useMutation({ mutationFn: async ({ id, input }: { id: string; input: WeightInput }) => requireData<WeightEntry>(await supabase.from('metric_entries').update({ numeric_value: input.value, recorded_at: input.recordedAt, metadata: metadata(input.note) }).eq('id', id).eq('user_id', userId!).select().single()), onSuccess: invalidate, retry: false })
